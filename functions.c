...
void ajouterVoiture(VOITURE *v) {
    ...
    scanf("%s", &v->marque);
    printf("Donner l'annee de fabrication de la voiture ");
    scanf("%d", &v->annee);
...
void afficherVoiture(VOITURE v){
    ...
    printf("Le modele de la voiture: %s\n",v.modele);
    printf("La marque de la voiture: %s\n",v.marque);
    printf("L'annee de fabrication de la voiture: %d\n",v.annee);
...
void ajouterClient (CLIENT * c,VOITURE *v,int nbVoitures){
   ...
   /// c->nombreLocations=0; (initialisation)
}
...
void afficherClient(CLIENT  c) {
    ...
    printf("Le nom et prenom du client: %s %s\n", c.nom, c.prenom);
    ...
    for (i = 0; i < c.nombreLocations; i++) {
        ...
        printf("Informations sur la voiture lou�e par le client :\n");
        printf("Code : %d\n", voiture.code);
        printf("Modele : %s\n", voiture.modele);
        printf("Marque : %s\n", voiture.marque);
        printf("Annee : %d\n", voiture.annee);
...
void miseAJour(VOITURE *tabVoitures, int nbVoitures, int codeVoiture) {
    ...
    printf("Veuillez entrer le code de la voiture a mettre a jour : ");
    scanf("%d", &(codeVoiture));
    ...
    printf("Saisissez le modele de la nouvelle voiture : ");
    scanf(" %s", &nouvelleVoiture.modele);

    printf("Saisissez la marque de la nouvelle voiture : ");
    scanf(" %s", &nouvelleVoiture.marque);

    printf("Saisissez l'annee de fabrication de la nouvelle voiture : ");
    ...
    for (i = 0; i < nbVoitures; ++i) {
        if ((tabVoitures+i)->code == codeVoiture) {
            *(tabVoitures+i) = nouvelleVoiture;
            int positionAModifier = i; // Position a modifier

         ///   modifierStructure("fichier_structure.dat", positionAModifier, nouvelleVoiture);
            printf("La mise à jour de la voiture avec le code %d a ete effectuee avec succes.\n", codeVoiture);
            return;
        }
    }
    printf("Aucune voiture avec le code %d n'a ete trouvee. La mise a jour a echoue.\n", codeVoiture);
}
void supprimerVoiture(VOITURE *tabVoitures, int nbVoitures, int codeVoiture) {
    ...
    for (i = 0; i < nbVoitures; i++) {
        ...
        if ((tabVoitures+i)->code == codeVoiture) {
            ...
            printf("La voiture avec le code %d a ete supprimee avec succes.\n", codeVoiture);
            voitureTrouvee = 1;
            break;
        }
    }
    ...
    if (!voitureTrouvee) {
        printf("Aucune voiture avec le code %d n'a ete trouvee. La suppression a echoue.\n", codeVoiture);
    }
}
...
void remplirTabResultat (RESULTAT ** tableauResultats,int nbResultats, CLIENT *c, int nbClients,int capaciteResultats){
        ...
        for (j=0; j<nbClients; j++){
            ...
            if (nouveauResultat == NULL) {
                printf("Erreur lors de l'allocation d'un nouveau resultat\n");
                exit(-2);
            }
            ...
            if (nbResultats >= capaciteResultats) {
                ...
                if (tableauResultats == NULL) {
                    printf("Erreur lors du redimensionnement du tableau de resultats\n");
                    exit(-3);
                }
            }

            // Ajouter l'adresse du nouveau resultat dans le tableau
            *(tableauResultats+nbResultats) = nouveauResultat;
            nbResultats++; // Augmenter le nombre de resultats
        }
}
...
void maxNbLocations(RESULTAT** tableauResultats, int nbClients){
    int indexMax = 0;
            ...
            // Afficher les details du client ayant le montant le plus élevé à payer
            printf("Client avec le nombre de location le plus eleve a payer :\n");
            printf("Nom et prenom du client : %s %s\n", (*tableauResultats+indexMax)->nomClient, tableauResultats[indexMax]->prenomClient);
...
{
     // Ecriture des informations du client dans le fichier
    ...
    // Ecriture des informations de chaque location
      int i;
     for ( i = 0; i < c.nombreLocations; ++i) {
...
CLIENT lireUnClientDuFichier(FILE *fp) {
  ...
  ///  c.locations = (LOCATION **)malloc(c.nombreLocations * sizeof(LOCATION** ));
    if (c.locations == NULL) {
        perror("Allocation de memoire a echoue");
        exit(-3);
    }
    int i ;
    for (i= 0; i < c.nombreLocations; ++i) {
